...
const create = async (
  ...
  if (!maxCapacity || typeof maxCapacity !== "number" || !Number.isInteger(maxCapacity) || maxCapacity <= 0)
...
const rename = async (id, newEventName) => {
  ...
  const eventCollection = await getEventCollection();
  const eventToUpdate = await eventCollection.findOne({ _id: new ObjectId(id) });
